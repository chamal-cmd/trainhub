...
export async function POST(req: NextRequest) {
  try {
    ...
    const { topicId, force = false } = await req.json()
    ...
    if (!force && topic.ai_quiz?.questions?.length > 0) {
    ...
    if (!contentText) {
      return NextResponse.json({ error: 'No content in this topic to generate questions from' }, { status: 400 })
    }

    const apiKey = process.env.ANTHROPIC_API_KEY
    if (!apiKey) return NextResponse.json({ error: 'ANTHROPIC_API_KEY not configured' }, { status: 503 })

    // Pick a random variation style so each generation feels different
