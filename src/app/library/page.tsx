...
import { CheckCircle2, Clock, Lock, Circle, ChevronRight, Trophy, Sparkles } from 'lucide-react'
    ...
    <div className="px-6 py-7 min-h-full bg-[#f8f8f8]">
          ...
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-full px-4 py-2">
            <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${modules.length > 0 ? Math.round((doneCount / modules.length) * 100) : 0}%` }}
              />
            </div>
            <span className="text-xs font-bold text-violet-700">
              {modules.length > 0 ? Math.round((doneCount / modules.length) * 100) : 0}%
            </span>
          </div>
        )}
      </div>
      ...
      {modules.length === 0 ? (
        ...
        <div className="max-w-2xl space-y-2">
          {modules.map((m, i) => {
            const { subject, completed, total, percent, quiz, quizPassed, fullyDone, quizPending, locked, readMins, dueDate } = m
            ...
            const card = (
              <div className={cn(
                ...
                state === 'locked'
                  ...
                  : state === 'done'
                  ? 'border-emerald-100 hover:border-emerald-200 hover:shadow-sm cursor-pointer'
                  : state === 'quiz_pending'
                  ? 'border-amber-200 bg-amber-50/20 hover:border-amber-300 hover:shadow-md cursor-pointer'
                  : state === 'active'
                  ? 'border-violet-200 bg-violet-50/20 shadow-md hover:shadow-lg cursor-pointer ring-1 ring-violet-200'
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className={cn(
                    ...
                    {state === 'locked'
                      ...
                      : <span className={cn(
                          'text-sm font-bold',
                          state === 'in_progress' ? 'text-violet-600' : 'text-slate-400 group-hover:text-violet-600'
                        )}>{i + 1}</span>
                    }
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {state === 'locked' ? (
                      ...
                      <p className="text-xs text-amber-600 font-medium mt-0.5">
                        All steps done — pass the quiz to complete this module
                      </p>
                    ) : (
                      <div className="flex items-center gap-3 mt-1">
                            ...
                            <div className="flex items-center gap-1.5 flex-1 max-w-[180px]">
                              <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${percent}%`,
                                    backgroundColor: state === 'done' ? '#10b981' : '#7c3aed'
                                  }}
                                />
                              </div>
                        ...
                        {dueDate && (
                          <span className={cn(
                            'text-[11px] shrink-0',
                            dueDate < new Date() && state !== 'done' ? 'text-red-500 font-semibold' : 'text-slate-400'
                          )}>
                  ...
                  <div className="shrink-0">
                    ...
                    {(state === 'in_progress' || state === 'not_started') && (
