                ...
                {withQuiz.map((subject: any) => {
                  ...
                  return (
                    <Link key={subject.id} href={`/admin/subjects/${subject.id}/quiz`}>
                      <div className="bg-white border border-slate-100 hover:border-indigo-200 hover:shadow-sm rounded-2xl p-4 flex items-center gap-4 group transition-all">
                        {/* Subject icon */}
                        <div
            ...
            <div>
              ...
              <div className="space-y-2">
                {withoutQuiz.map((subject: any) => (
                  <Link key={subject.id} href={`/admin/subjects/${subject.id}/quiz`}>
                    <div className="bg-white border border-dashed border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30 rounded-2xl p-4 flex items-center gap-4 group transition-all">
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-500 group-hover:text-indigo-700 transition-colors truncate">
                          {subject.title}
                        </p>
                        <p className="text-xs text-slate-400 mt-0.5">No quiz created yet</p>
                      </div>
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 group-hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors shrink-0">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-2xl border border-slate-100 p-4 text-center">
              <p className="text-2xl font-bold text-indigo-600">{withQuiz.length}</p>
          ...
          {recentAttempts && recentAttempts.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-50">
                ...
                <Link href="/admin/reports" className="text-xs text-indigo-600 font-semibold hover:underline">
                  View all →
                </Link>
              </div>
          ...
          <div className="bg-indigo-50 rounded-2xl border border-indigo-100 p-4">
            ...
            <ul className="space-y-2 text-xs text-indigo-700/80">
